...
- 状态事件统一为版本化 `TASK_STATE_UPDATE`：主业务事务提交后才以独立事务写状态，该状态事务提交后才发布；状态失败只告警，批改页按版本丢弃旧消息、合并刷新并每 10 秒 REST 校准，教师首页也每 10 秒校准操作题状态。课堂大屏另每 10 秒调用 `GET /business/classroom-state/summary` 获取全班真实任务汇总；新增 SQL 未执行、正式服务未发布。
