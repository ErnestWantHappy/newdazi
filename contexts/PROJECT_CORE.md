...
## 2026-09-03 P2 通用分组与学生桌面（源码已实现，未迁移/未发布）
...
- Vue3 班级管理每行增加“学生桌面”入口，桌面支持终端/作业状态视图切换、默认学号网格和教师个人拖动布局。协作轨迹抽屉、分组拖动编辑、在线协作房间路由留待 P3。
- 已验证：`mvn -pl ruoyi-business -am -DskipTests compile` 成功；`npm run build:prod` 成功；`git diff --check` 无错误。SQL 尚未执行，本地数据库和浏览器冒烟待做。
...
总需求、总体设计、分阶段任务、开发提示词和新任务交接提示词位于 `contexts/multi-feature-upgrade-20260902/`。P0-A/P0-B 与 P1-A/P1-B/P1-C 的核心代码已完成本地纠偏；P1-D 核心状态链已实现，但全页面接入与环境验收未完成；P2、P3-PoC、P3 尚未开发。
...
- P0-P1 已进入本地代码实施，但新增任务状态 SQL 尚未执行，也未发布；P2/P3 仍只完成方案。正式网络能否观察到每台学生机独立连接 IP，以及 CryptPad/OnlyOffice 能否提供比“小组版本差异”更精确的作者数据，仍必须通过后续 PoC/机房验收确认。
