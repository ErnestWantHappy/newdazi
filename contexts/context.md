...
> **当前焦点（2026-08-07：操作题多格式作品与 AI 草稿批改已发布 10.52.1.123；真实教师 Key 的无学生数据视觉连通已验收，学生作品批量调用仍受隐私与盲测门禁约束）**
...
> 12. **GitHub 与正式发布事实**：代码已提交并推送 `codex/research-activity-v1`，发布提交为 `9328868`，现有草稿 PR #3 已包含该提交。服务器先备份 `ry-vue` 与当前 release：备份目录 `D:\program\3009dazipingtai\backups\20260807_152125_9328868_operation_ai`，SQL dump 65,736,374 bytes、SHA-256 `6F4F78B074189FE5E7853CE56F257A950AB956E8247D3A5573FC132F6B6FDBC2`，清单 SHA-256 `50D7F9354E2CD89B6921A87176345A40CD3B09C5A551AE6EA58911FD02B5590D`。随后按 P2→P3→P4→P5 执行正式库迁移并后检：作品/版本/附件均 12,732，题目材料 95，评分快照 106，重复、孤儿、未绑定快照和非法 AI 结果均为 0。当前 release 为 `20260807_152632_9328868`，上一回滚点 `20260803_212451_a694428` 保留；3009/3010 均为 200，80 端口站点未改动，部署后后端 ERROR 与 stderr 非空行均为 0。
...
> 14. **仍未解除的产品门禁**：本轮只验证无学生数据的视觉连通性，没有把任何真实学生作品发送给模型，也没有创建班级 AI 任务，因此不得宣称准确率、证据质量、批量稳定性、成本或隐私合规已验收。学校授权、百炼数据留存/训练策略审查、备份日志审查和千问/豆包匿名同样本盲测完成前，真实学生作品批量调用继续禁止。本机克隆中的 44 份历史 Word 和唯一既有区域操作题物理文件缺失，历史批量转换与区域实体样本仍待补验。
...
| 当前开发分支 | `codex/research-activity-v1`（操作题发布提交 `9328868` 已 push，草稿 PR #3 已包含；用户要求不再新建分支） |
...
| 前端 3010 | **当前** `20260807_152632_9328868`；上一应用回滚点 `20260803_212451_a694428` 完整保留；80 端口站点未改动 |
