...
> **当前焦点（2026-08-09：操作题 AI 批改 v2 闭环已发布 10.52.1.123；范围确认、参考答案必填、刷新恢复、逐项展示和安全批量采用已完成，真实学生作品继续受隐私与盲测门禁约束）**
...
> 12. **GitHub 与正式发布事实**：代码已提交并推送 `codex/research-activity-v1`，现有草稿 PR #3 已包含操作题 AI v2 提交 `0d93535` 及此前发布/恢复提交。当前正式 release、备份、制品哈希和验收事实以第 16～17 条为准；`20260807_165505_4dcb696` 保留为直接应用回滚点。
...
> 17. **正式班级只读回归事实**：郑东旭教师在课程 237、2024 级 6 班、题目 1281 上刷新及离开后重进均恢复 `AI 已暂停 13/44`，旧任务警告和禁用批量采用正确；成功样本显示第一题 15/15、第二题 15/15、总分 30，页面错误为 0。正式库仍为 44 份提交、43 份人工已批、1 份未批；任务 2 仍为 PAUSED，13 条 SUCCESS、31 条 PENDING、44 条均 NOT_APPLIED，教师参考答案记录为 0。本轮未取消旧任务、未上传虚假参考答案、未调用模型处理真实学生作品、未改任何正式成绩；下一步必须由教师提供真实参考答案，并在隐私门禁解除后重新建任务。
...
| 当前开发分支 | `codex/research-activity-v1`（当前服务器代码提交 `0d93535` 已 push，草稿 PR #3 已包含；用户要求不再新建分支） |
...
| 前端 3010 | **当前 release** `20260809_121339_0d93535`；上一版 `20260807_165505_4dcb696` 与更早回滚点均保留；80 端口站点未改动 |
