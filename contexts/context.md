...
## 2026-07-11 双功能开发完成记录（以此节为当前事实）

### 数据库与迁移状态

- 当前本地数据库为 MySQL 8.0.39、`xueyeceping1`；`sql/county_exam_integrity_fix.sql` 已连续执行两次并验证幂等，当前库不需要再次执行。
- `accuracy_rate`、`completion_rate` 均为 `decimal(7,4)`；区域抽测评卷教师唯一键为 `(exam_id, question_id, grader_id)`。
- 禁止向当前库整库导入包含 `DROP TABLE` 的 `sql/ry-vue260709.sql`。新建数据库时按 `xueyeceping1-07-11.sql` → `guide_sheet_v1.sql` → `county_exam_integrity_fix.sql` 执行。

### 电子导学单验收结论

- 已在空 `GUIDE_SHEET_AI_API_KEY` 下完成创建、设计、发布、学校/入学年份/班级指派、草稿恢复、提交、无密钥转人工、人工评分、看板和 Excel 导出全流程；外部 AI 与教师助手关闭时不影响核心保存和评分。
- 学生同时存在区域抽测和导学单时，后端写接口与前端路由均以区域抽测优先；抽测关闭后导学单立即恢复。
- 保存请求使用真实 `AbortController.signal`；卸载兜底使用 `VITE_APP_BASE_API`，不再硬编码 `/dev-api`。
- 课堂 WebSocket 使用可配置的精确 Origin 白名单。合法学生经 Vite `/ws` 代理和直连 8080 均返回 101；无 Cookie、错误部门、错误入学年份、错误班级和恶意 Origin 均返回 403。
- 测试导学单 `sheet_id=1` 已由创建人关闭并删除；`biz_guide_sheet`、`biz_guide_sheet_assignment`、`biz_guide_sheet_answer`、`biz_guide_sheet_upload`、`biz_guide_sheet_progress` 中相关记录均为 0。

### 区域抽测验收结论

- `exam_id=5` 已完成两名学生最终提交：学生甲正常提交并取得 100 分（理论 10、打字 10、操作 80），学生乙由关闭流程自动提交并取得 0 分。
- 操作题 `question_id=1287` 的 100% 评分项在 80 分题目上显示 `/80`；伪造 81/80 分项请求被后端拒绝，正常 80 分提交成功。
- 草稿、最终提交、超时自动提交均锁定参考学生记录并使用提交 CAS。新增测试确认答案写入后 CAS 返回 0 时 Spring 事务执行回滚且不提交；所有自动提交公开入口均在事务内。
- 成绩已发布并显示正式分析标识；学校平均分排名、百分制分数分布、各题表现、学生明细与 Excel 导出均已验证。学校汇总为 1 校、2 人、平均分 50、及格率 50%。

### 构建与质量结论

- 已执行 `mvn -pl ruoyi-admin -am clean package`，完整业务测试 23 个全部通过，并生成最新 fat jar。
- 已执行 `npm run build:prod`，构建成功；仅保留第三方 `vform3-builds` 的既有 `eval` 构建提示。
- `git diff --check` 通过；新标签页复查匿名评卷与导学单列表，控制台无 error/warning。
- AI 密钥继续只允许由部署环境注入；旧密钥未恢复，仓库不保存明文密钥。
...
**按题评卷配置**：
教研员或管理员按区域抽测中的操作题选择评卷教师，并可设置每位教师批改的答卷份数；未设置份数时，由系统把该题剩余答卷自动均分。
