...
> **当前焦点（2026-07-28：教研活动准备 push 并部署 10.52.1.123）**
> 1. **本轮路线已确认**：在现有分支 `codex/research-activity-v1` 完成交接规范、功能与文档审查和全套回归，拆分功能/规范提交并 push；不新建分支、不创建 PR。
> 2. **功能状态**：课堂反思改名、图片多选按顺序批量上传、单张失败不阻断、拖拽缩放及保存/刷新/再编辑回显已在本机验证；图片预览统一走 `/common/resource/view`，旧正文 URL 由幂等 SQL 修复。
> 3. **当前 Git 状态**：功能提交 `9019cce` 已完成，规范与最终交接上下文正在提交；`origin/codex/research-activity-v1` 仍为 `900da10`，尚未 push。根目录 `uploadPath-private/` 为本机运行数据，必须保持不删除且禁止提交。
> 4. **发布门禁已通过**：2026-07-28 后端 clean package（业务 193/193、admin 3/3）、前端纯函数 7/7、生产构建、批量上传/缩放 8/8、图片闭环 15/15、浏览器主流程 15/15 均通过；`[AI测试]` 主题/留言和孤立通知/资源余量均为 0。下一步提交并 push 后，以远端最终提交为唯一基线部署 10.52.1.123。
...
| 当前开发分支 | `codex/research-activity-v1`（本轮功能提交 `9019cce`；远端仍为 `900da10`；用户要求不再新建分支） |
...
### 2.8 教研活动（2026-07-28 准备 push 并部署 10.52.1.123）
...
- **角色边界**：教研员、教师、学生已做浏览器与 API 验收；学生无菜单、直达路由回学生首页、接口 403。管理员因本地文档无可用账号且禁止猜密码，本轮未单独登录；教研员已覆盖管理操作，管理员仍列入服务器发布冒烟。
...
- 发布边界：本轮仍在现有分支 `codex/research-activity-v1`，未 push、未创建 PR、未发布 3009/3010。本机阻断已解除，具备进入内网发布流程的条件；发布时须先备份并计算 SHA-256，执行教研活动建表/活动时间/多校回填及 `research_activity_image_url_fix.sql`，给 Nginx 3010 配置 `client_max_body_size 60m` 并 `nginx -t`，发布到新 release 后做管理员/教研员/教师/学生冒烟。应用回滚切回旧 release，新表保留不 DROP；正文 URL SQL 为向前兼容修复，无需回滚。
