...
> **当前焦点（2026-07-22，实现中：已毕业课不可见 + 区域抽测评卷）**  
> 1. **P0**：教师首页去掉 `gradeId > 0` 才装课的误杀（已毕业 `gradeId=-1` 导致空卡片）。  
> 2. **P0/P1**：评卷教师池可发现性、配置弹窗「一键均分」、关闭后生成任务文案；入口仍为待评>0 才显示。  
> 3. **交付**：本机修验 → `10.52.1.123` 备份代发布 3009/3010 → Playwright。  
> 4. **已确认产品规则**见 §2.1 / §2.4；学年热修与菜单三连修仍有效。
...
| 当前开发分支 | `codex/hotfix-academic-year-20260721`（线上学年热修同源） |
...
| 前端 3010 | `20260722_113044_8e24c69_menu-ui`；旧 `20260721_212007_1d0e7ef` 保留可回滚 |
...
- **2026-07-22 已毕业课/评卷修复交付（方案 A）**：本机修 + 单测/构建通过 → 正式库/发布前备份 + SHA-256 → 新 `releases/<时间戳>_<hash>/` 切换 NSSM 3009 与 Nginx 3010 → Playwright 线上验收；P0 阻断本轮修，P1 记入本文件。

### 2.8 教研活动（2026-07-22 规划已确认，待开发）
...
- 搜索分活动主题和课程资源；课程资源默认优先，支持关键词以及学段、年级、学期、第几课、作者、时间等结构化筛选，不引入 Elasticsearch。
- 通知形式为不通知、普通站内通知、重要站内通知；范围可按一个或多个学段，或单选/多选指定教师。通知范围只控制首页提醒，不限制主题可见性；接收教师在 **教师首页最顶部置顶通知栏** 看到最近未读通知，点击标记已读并直达主题。
