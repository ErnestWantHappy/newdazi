...
> **当前焦点（2026-07-23：已毕业课/评卷修复已上线 r2）**
> 1. **已发布** `20260723_graduated_grading_r2`（3009/3010）：已毕业届装课误杀修复；评卷一键均分；评卷池多校展示；入口仍为待评>0。
> 2. **线上验收**：教师 `19157727791` 小学部 2020 已毕业 **13** 门课；评卷池「郑东旭」展示小学部+初中部双校；弹窗含一键均分。报告 `output/playwright/server_graduated_grading_smoke_report.json`。
> 3. **说明**：郑东旭并非两个 `sys_user`，而是 **同一账号多校选校**；配置评卷选该账号即可覆盖两部。
> 4. **教研活动**本机能力见 §2.8；已修复多校教师按学段漏投，并增加双时间筛选和卡片活动时间。当前已知阻断问题：富文本上传图片后正文不显示；下一轮先做全功能模拟测试并定位图片上传、鉴权读取与 HTML 回显链路（仍未发布内网）。
...
| 当前开发分支 | `codex/research-activity-v1`（基线 `730d3f2`；用户要求后续不再新建分支） |
...
| 前端 3010 | **当前** `20260723_graduated_grading_r2`；可回滚 `20260723_graduated_grading` / `20260722_113044_8e24c69_menu-ui` / `20260721_212007_1d0e7ef` |
...
### 2.8 教研活动（2026-07-23 本机功能实现完成，待全量模拟测试与问题修复）
...
- 主题及三类留言均使用完整富文本，至少支持文字、表格、图片与超链接。留言类型为 **普通留言 / 活动纪实 / 课程资源**；只有课程资源强制填写学段、年级、学期、第几课和课程标题。
...
- 编辑器：公共 Quill 2 仅增加默认关闭的可选表格/上传参数；教研活动启用表格、JPG/PNG/WebP、单图 10 MiB、最多 20 图。表格改为一键插入 3×3，编辑区补齐可见网格、单元格尺寸和焦点反馈；表格插入与编辑已通过专项验证，但图片端到端链路仍有下述已知问题。
- **已知阻断问题（2026-07-23 用户实测）**：富文本选择并上传图片后，图片未在编辑器或正文中正常显示。尚未确认故障发生在上传响应 URL、私有图片鉴权读取、HTML 清洗/持久化、详情回显或浏览器资源请求哪一层；禁止在没有网络响应和持久化数据证据时直接下结论。下一轮必须先稳定复现并完成“上传即时预览 → 保存 → 刷新详情 → 再编辑”的闭环验证。
- 已完成验收：2026-07-23 后端 clean package、教研活动专项 32/32、前端纯函数 7/7 与生产构建通过；多校投递/双时间筛选专项浏览器与 API 冒烟 12/12，真实教师 `19157727791` 在初中部首页看到主题 15 及 `2026-07-31 00:00:00`，报告 `output/playwright/research-activity-multischool-filter-smoke.json`。该结果不代表教研活动全部功能已完成端到端验收。
- 下一轮验收范围：按管理员/教研员/教师三角色覆盖活动主题、交流分享、三类留言、表格、图片、链接、附件、定向通知、多校切换、双时间筛选、详情返回、编辑/隐藏/置顶/再次通知、未读状态与权限边界；优先修复图片不显示，并为每个失败保留页面截图、请求/响应、控制台错误和数据库证据。
...
- 发布边界：本轮代码提交到现有 GitHub 分支 `codex/research-activity-v1`，不新建分支、不创建 PR；没有发布 3009/3010。图片阻断问题修复并完成全量验收前，不得发布内网。进入内网发布时须先备份并计算 SHA-256，执行 SQL，给 Nginx 3010 配置 `client_max_body_size 60m` 并 `nginx -t`，发布到新 release；应用回滚切回旧 release，新表保留不 DROP。
