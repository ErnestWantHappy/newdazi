...
> **当前焦点（2026-08-07：操作题多格式作品、统一页图、评分快照与 AI 草稿批改已完成本机代码；真实 AI 调用和服务器发布仍受门禁约束）**
...
> 7. **P3 统一页图底座已完成**：`biz_practical_attachment` 与 `biz_county_exam_answer` 均使用独立规范化状态、页图 JSON、渲染器版本、重试次数/时间和错误信息；Office 先经 LibreOffice 转 PDF，PDF 再由 PDFBox 2.0.32 按 120 DPI 输出最多 50 页 JPEG，图片安全解码、白底和最长边 1800 像素归一化。普通课程和区域抽测教师页均优先连续展示规范化页图，原附件/PDF 与授权下载保留为降级通道。教师端异常重转已覆盖全部新附件类型，并只接受失败或超过 10 分钟的卡住任务，正常转换和已达 3 次上限的附件不会重复入队。
...
> 9. **P3 迁移与验收证据**：本机已执行 `sql/operation_preview_pages_v1.sql` 及后检；迁移前备份 `D:\dmwprogram\newdazipingtai\backups\20260804_182235_local_before_operation_preview_pages_a694428\xueyeceping_server_20260729_before_operation_preview_pages.sql`，73,177,353 bytes，SHA-256 `7BAAE90D157A1CAED6C75729A8B60E1E459F04C5E6E2F64E0A3618CC5C631ED2`。真实 PNG 1 页、DOCX 9 页、PDF 1 页均成功，同 PDF 缓存命中 0.64 秒且 URL 独立；另用真实生成的 PPTX（2 页）和 XLSX（1 页）经 LibreOffice 转 PDF 后逐页渲染并目视确认中文、表格和页序正常，样本与证据在 `output/test-fixtures/`。区域抽测代码、Mapper、状态、重试与教师连续页图已复用同一渲染器；本机唯一既有区域操作题物理源文件缺失，因此保留为明确实体环境门禁。
...
> 11. **P5 AI 草稿批改代码已完成并默认关闭真实调用**：主供应商采用阿里云百炼，默认模型 `qwen3.7-plus`，`qwen3.6-flash` 为降本选项；豆包只用于上线前匿名同样本盲测。新增教师配置、班级任务、逐份结果 3 表；Key 使用部署主密钥派生的 AES-256-GCM 加密，前端只见末四位且可替换、连通测试、删除。后台独立线程池支持进度、逐份失败隔离、暂停/继续/取消/失败重试；输入不含学生姓名/学号，包含评分快照、规范化作品页图和可渲染参考材料。输出使用 `operation-rubric-v1` 严格 JSON，服务端重新校验评分项集合、逐项上限、合计和总分；建议只填入教师评分框，仍须教师点击原有提交按钮，绝不直接写正式成绩。
> 12. **P5 迁移、安全与测试证据**：本机已执行 `sql/operation_ai_grading_v1.sql` 及后检，3 表存在、非法结果 0、非 `v1:` 密文 0；迁移前备份 `D:\dmwprogram\newdazipingtai\backups\20260804_191259_local_before_practical_ai_a694428\xueyeceping_server_20260729_before_practical_ai.sql`，73,780,034 bytes，SHA-256 `56F93C92F233677F3E2B21B62D0FFFF6A8CB7C0DF7C1F7659F1F32521F827EEA`。业务模块全量 253/253，admin clean package 3/3，Vue3 2678 modules 生产构建均通过；教师接口用一次性本机主密钥与假 Key 验证保存后数据库仅有 `v1:` 密文、接口仅回末四位、删除后记录为 0。最终服务以“无部署主密钥、无教师 Key”的安全默认状态启动；设置弹窗在 Key 输入框下提供百炼官方 API Key 控制台的新标签页入口。教师 AI 设置页浏览器冒烟 11/11，无页面错误和 500，报告 `output/playwright/operation-ai-grading-ui-smoke.json`，截图 `operation-ai-grading-settings.png`。没有教师真实 API Key，不得宣称真实模型连通、准确率或成本已验收。
> 13. **剩余门禁与发布边界**：真实学生作品批量调用仍禁止，必须先配置 `PRACTICAL_AI_MASTER_KEY`、完成学校授权/供应商数据留存与备份日志审查，再以匿名同样本盲测千问和豆包并记录准确率、证据、失败率、时延与成本。本机克隆中的 44 份历史 Word 和唯一既有区域操作题物理文件缺失，不得宣称历史批量转换成功。本轮未 commit、未 push、未发布 `10.52.1.123`；正式服务器仍是 `1488206`。发布须按 P2→P3→P4→P5 顺序备份并执行 SQL、发布新 jar/dist、设置部署主密钥、重启后端并完成三角色及回滚验收。
...
| 前端 3010 | **当前** `20260728_151733_1488206`；上一应用回滚点 `20260723_graduated_grading_r2` 完整保留 |
...
- **届别锚点**：`biz_lesson.entry_year`（入学年份，稳定）；`grade` 仅为创建时内容年级快照。全平台学年切换日 **每年 7 月 20 日**。
