...
## 2026-09-02 分组协作升级待开发

- [ ] G1 完成通用班级分组方案、成员、组长和课时快照的数据模型与 ADR。
- [ ] G2 建立教师端学生桌面，分离“调整座位”和“调整分组”模式；组长仅视觉标记。
- [ ] G3 将协作配置从单一起始文件扩为多个非计分任务版本，并建立小组到任务版本的显式映射。
- [ ] G4 将房间边界扩为课程、班级、课时分组和任务版本；旧房间兼容迁移为全班组且保留历史版本。
- [ ] G5 学生入口按课时快照只返回本人小组房间，覆盖同组、异组、跨班和跨校权限测试。
- [ ] G6 明确学生操作轨迹的粒度和留存规则，再实现事件采集、查询接口与教师时间线。
- [ ] G7 验证在线协作全程不写个人答案和成绩，不影响课程总分及自动推进。
- [ ] G8 完成多小组、多文件、多人同时编辑、断线恢复、历史房间回归和容量验收。
- [ ] G9 实现会话进入/离开/心跳/保存事件表与学生轨迹抽屉，默认保留一个学年。
- [ ] G10 实现相邻文档版本异步差异提取：DOC/DOCX、XLS/XLSX、PPT/PPTX 分级摘要，失败不得阻塞保存。
- [ ] G11 在 UI 和审计中严格区分“触发保存者”与“该版本变化的实际作者”，未解决精确归属前不得作个人贡献结论。
- [ ] G12 将协作从计分操作题解耦为独立课堂协作活动，并提供从既有操作题材料导入入口。
