...
- [x] C4a 保存失败回调补充错误对象；前端保留未保存状态，避免 CryptPad 将失败保存误判为成功。
- [x] C5 接入 Vue3 教师设置入口、教师编辑页和学生班级入口。
- [x] C6 完成幂等 SQL、后端专项测试、业务测试 308/308 和 Vue3 生产构建。
- [x] C6a 审计补强：IoT 口令密钥改为 `IOT_PASSCODE_SECRET` 外置注入；学生物联概览增加当前课程门禁；专项测试通过。该项不执行 SQL、不重启服务。
- [x] C7 完成扩展服务器 Docker/Nginx/UFW/Compose/systemd/备份目录和固定版本源码构建准备。
- [x] C8 本机基线库在完整备份后执行 `sql/cryptpad_collaboration_v1.sql`，WPS 历史房间保持不变；正式服务器仍须单独前检、备份和迁移。
...
- [x] 课程设计器“文件作品”行内开启协作、教师首页按课程选择班级入口、低版本 Chrome 语法门禁和稳定参与者 ID 已在代码中完成；复用既有 `biz_collab_room` 和 CryptPad 成员列表，不新增表。
  - [x] 教师首页协作入口布局已修复：入口从课程信息窄栏移到卡片固定底部操作条，课程标题、班级标签、课次和右侧原操作不再互相挤压；2026-08-20 前端独立发布到 `20260820_collaboration_entry_compat_v3`，生产构建、Nginx 候选/正式配置检查、reload、3010 首页和验证码接口 HTTP 200 均通过。正式后端保持 `20260820_iot_scheme_a_v1`，无 SQL、无后端重启；自动浏览器登录因生产验证码开启未完成视觉截图，需教师强制刷新后现场复核。
...
- 本机代码测试、角色页面冒烟和 CryptPad 结构迁移均已完成并留有备份/后检证据。
- 按用户要求，HTTP/WS 继续作为临时验收态，HTTPS/WSS 不在本轮修复范围。
- C10/C11 的同班多人编辑、刷新保留、平台保存版本递增及容量门禁仍需真实同班账号和隔离环境；不得以跨班账号或 `/checkup/` 200 代替。
...
## 2026-08-21 协作体验修复（已发布，release `20260821_iot_course_switch_v1`）

- [x] 修复「协作文档版本已变化，请刷新后重试」频繁出现：保存链串行化、版本 CAS 失败时重新获取会话并重试一次，失败不再假装成功；随本次发布上线（此前 3010 运行的是未带重试的旧构建，每 15 秒自动保存都带旧版本号必然报错）。
- [x] 修复 OnlyOffice 参与人身份显示（学生 a 编辑却显示学生 b / 教师只看到自己）：CryptPad 2026.5.1 的 `getParticipants()` 优先使用本地 integrationConfig 用户名，已在服务器侧以只读挂载补丁覆盖 `inner.js`（`/srv/cryptpad/patches/inner.js`，优先 `users[nId].name`）；compose 备份 `docker-compose.yml.bak-20260821`，容器重建后 healthy，公网入口已验证返回补丁内容。
- [x] 编辑器新增在线成员列表（含「我」与「只读」标识）、版本状态 tag 与重新加载；教师房间列表新增「版本历史」（`GET /business/collaboration/room/{roomId}/revisions`，教师/管理员权限）。
- [ ] 待真实同班账号复测：多人同时编辑时各自身份显示正确、保存回平台版本递增与刷新保留。

## 2026-08-24 首次保存修复

- [x] 新课程开启在线协作时，在保存前识别文件作品候选；单候选自动选、多候选弹窗选、取消则关闭。
- [x] 不再依赖先生成 lessonId 后重新进入；Vue3 生产构建通过并随 v1.26.1 发布。
- [ ] 生产不新增临时课程，首次保存与房间生成留待教师下一次真实建课时抽查；既有同班多人编辑验收项不因此视为完成。

## 2026-08-24 设计器协作入口收口（v1.26.2）

- [x] 在线协作入口移到物联网下一行，说明收进 tooltip，默认布局更紧凑。
- [x] 开启前强制校验文件作品操作题与可编辑 `STARTER`；零/单/多候选分别提示、自动绑定或弹窗选择。
- [x] 已选题变化后重新核验绑定，避免题目移除后保留失效协作配置；Vue3 生产构建和线上教师页面冒烟通过。
