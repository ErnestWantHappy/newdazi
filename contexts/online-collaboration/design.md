...
## 9. 2026-09-02 分组协作目标模型（已确认部分）

```text
班级可复用分组方案
  -> 课程指派时冻结课时分组快照
  -> 一个非计分协作活动配置多个任务版本/起始文件
  -> 每个小组绑定一个任务版本并创建独立房间
  -> 学生只取得本人小组房间
```

- 不复用 `biz_iot_group` 作为通用分组事实；物联网分组绑定实验配置，生命周期和权限不同。通用分组应建立独立领域，物联网后续如需共用只能显式引用快照或通过适配层同步。
- 座位布局与分组成员关系分开保存。教师端提供“调整座位”和“调整分组”两个明确模式，避免普通拖动意外改变学生所进入的协作文档。
- 现有房间唯一键是 `lesson_id + question_id + dept_id + entry_year + class_code`，升级后必须增加课时分组/房间组维度；旧班级房间迁移为兼容的“全班组”，不得覆盖或删除历史 revision。
- 多份起始文件建模为协作任务版本，并由小组映射表绑定；不建立多条计分课程题目，也不写成绩表。
- 学生操作轨迹由平台记录身份、房间、进入/离开、心跳、保存触发和保存结果；文档版本完成后由异步差异任务比较相邻版本，不能阻塞协作文档保存。
- 当前浏览器 `onSave` 返回的是多人协作后的完整文档，平台能确定触发保存的会话，但不能据此断言这批内容变化全部由该学生完成。界面必须把“触发保存者”和“版本内容变化”分开表达，精确作者归属仍是最后一轮门禁。
- 项目已经依赖 Apache POI 的 OOXML/旧格式组件，可用于 DOCX 段落与表格文本、XLS/XLSX 单元格、PPT/PPTX 幻灯片文本的结构化提取。格式、图片、图表、宏和复杂对象只提供受限摘要或“内容已变化、暂无法结构化描述”，不得伪造精确差异。
- 差异结果按版本保存，UI 默认按学生课堂时段聚合，并允许展开到具体 revision；默认留存一个学年，第一阶段不导出。
- 通用分组支持平均、学号区间和手动三种生成方式。保存时校验成员不重不漏，每个非空组恰有一名视觉组长；缺席状态不改变成员关系。
- 协作活动从计分操作题中解耦，独立保存活动、任务版本、小组映射和房间。可从操作题材料复制创建任务版本，但不引用其分数和答案关系。
- 首名学生进入后冻结活动配置；换组创建受审计的成员路由变更，保留旧房间内容和轨迹，默认撤销学生对旧组房间的访问。
