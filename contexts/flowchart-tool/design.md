...
- 课程设计器：把 `FLOWCHART` 展示为“画程流程图”，沿用操作题选择与分值；添加前必须确认基础图和标准答案均有有效节点，未完成或旧格式异常配置不得选入课程。
- 题库管理和课程设计器：通过只读预览接口展示学生基础图；公开题预览仅返回基础图和 `configReady`，标准答案和规则仍只允许创建者或管理员通过编辑配置接口读取。
- 学生首页：流程图题显示“打开画程”，不显示文件上传；弹出或全屏编辑后自动保存并明确提交。
- 教师批改：流程图题不走 Office/PDF 预览，改为只读画布＋检查面板；AI 任务自动渲染学生图和课程标准答案图，图片为主要依据，JSON/结构结果为辅助上下文；分数保存继续走现有接口。
...
只允许 `terminal`、`process`、`decision`、`inputOutput` 四类节点及 `polyline` 连线。服务端重新解析并输出规范 JSON；未知字段、HTML、URL 和外部资源拒绝或剔除。节点、连线文字禁止完整 HTML 标签、HTML 注释和 `javascript:` 协议文本；错误必须指明基础图或标准答案的具体节点/连线位置，普通比较表达式不应被误判。
...
- `PracticalAiJobService` 按 `practicalMode=FLOWCHART` 建立同一套操作题 AI 任务，不要求教师上传普通文档参考附件。
- `PracticalAiJobWorker` 为每个流程图结果渲染学生作品图和课程标准答案图，并把提交 JSON、答案 JSON、结构检查结果和规则快照作为辅助上下文传给视觉模型。
- 流程图结果以 `biz_flowchart_submission.submission_id` 作为非空版本锚点；处理前后都校验 `FLOWCHART:<submissionId>` 引用，补交会使旧结果失败。
- `QwenPracticalVisionGradingProvider` 使用与文档题相同的 JSON 输出契约和分值校验，但明确图片主判、结构信息辅助、证据页码只指向学生作品。
- 前端沿用普通文档题的任务进度、暂停、重试、批量采用和审计展示；流程图面板额外展示 AI 建议分、置信度、总评和“采用到评分框”。
...
- `GET /business/flowchart/question/{questionId}/preview`：读取选题预览，仅返回学生基础图和不泄露答案的 `configReady`；创建者、管理员或公开题可访问。
...
1. 服务端把已提交的学生 JSON 与课程快照中的标准答案 JSON 渲染为 JPG；渲染图只用于 AI 识别，不替代 JSON 真实数据。
2. 模型输入顺序固定为学生作品、标准答案图，以及可选的空白起始材料；流程图 JSON、结构检查结果和规则快照以结构化辅助上下文传入。
3. 模型必须按现有评分项返回完整 JSON、逐项分数、学生作品页码证据、总评、置信度和人工复核标记；服务端校验分值范围、总分和评分项完整性。
4. AI 任务结果只写 `PracticalAiResult` 建议字段，不写正式成绩；教师复核后才调用原操作题评分接口。
...
### 2026-09-07 AI 回显与解析热修

- 提交列表 `practicalVersionId` 保持文件版本语义，`flowchartSubmissionId` 为流程图版本；前端按 `practicalMode` 选择匹配字段，两边版本缺失不匹配。不要以 COALESCE 混合两个 ID，以免触发文件附件/评分快照补全。
- Mapper 按 `student_answer = CONCAT('FLOWCHART:', fs.submission_id)` 精确关联当前提交，避免同一答案 ID 的历史提交重复关联。
- 空评分项忽略模型冗余项，保存空分项得分与原始总评证据；总分必须为可转 int 的 JSON 整数且在题目满分内。有分项时继续完整性及总分一致性校验。
- 页面恢复最新任务并复用现有建议卡、单份填分、批量采用；批改权限、期限、版本校验与覆盖审计保持不变。自动聚焦显式声明评分框及工作区引用。
...
- AI 专项：流程图批量生成→队列进度/暂停/重试→视觉建议展示→单份或批量采用前的版本校验；验证学生补交不会污染旧结果。
