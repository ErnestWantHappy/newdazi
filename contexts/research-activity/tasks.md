...
- [ ] 7. 通知后端
  ...
  - [ ] 7.2 实现学段接收人扩展
    - 按 `sys_dept.school_type` 选择启用 teacher 账号。
    - 多账号分别通知；同主题+账号唯一去重。
    - 每批最多 500 行写入。
    - _Requirements: REQ-09_
    - _Dependencies: 7.1_
...
| UAT-03 | 教研员按小学+初中发布重要通知 | 两学段启用教师账号各一条未读 |
| UAT-04 | 教师首页打开 | 通知栏为第一内容块，重要通知优先 |
