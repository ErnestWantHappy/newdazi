...
活动通知分享使用 32 字节随机令牌，数据库只保存 SHA-256 哈希。匿名接口仅返回通知标题、清洗后的正文 HTML、活动时间、发布人、学校和发布时间；评论、课程资源、附件、云盘地址/提取码、接收人及内部 ID 均不返回。正文图片必须通过带令牌接口读取，并验证图片 URL 确实存在于该通知正文中。
