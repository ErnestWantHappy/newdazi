...
> 2026-08-07 发布说明：正式服务器 release 为 `20260807_152632_9328868`。服务器生成的部署主密钥通过 NSSM 服务环境注入，教师 Key 保存为 `v1:` 密文；郑东旭教师账号在 `deptId=169` 使用无学生数据测试图调用 `qwen3.7-plus` 成功，服务重启后直接复用既有密文再次成功。该验证只证明 Key、网络、视觉输入、结构化输出和主密钥持久化，不代表真实学生作品的准确率、隐私门禁、批量时延或成本通过。
...
- 应用回滚优先切回 `20260803_212451_a694428` 的 NSSM AppDirectory/AppParameters 与 Nginx 3010 root；P2～P5 新表和兼容列可保留，不需要为应用回滚立即还原数据库。
